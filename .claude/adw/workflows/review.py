"""
Review workflow - Reviews implementation against Linear issue requirements.

This workflow:
1. Loads workflow state
2. Checks out the correct branch
3. Executes the /review command with issue ID
4. Parses review results
5. Optionally fixes blocker issues and re-reviews
"""
...
def run_review(state: WorkflowState, fix_blockers: bool = True) -> int:
    ...
    while attempt < MAX_FIX_ATTEMPTS and not review_passed:
        ...
        # Step 2: Execute /review command with issue ID
        ...
        # If blockers found and we can retry, fix and re-review
        if fix_blockers and attempt < MAX_FIX_ATTEMPTS and blockers:
            logger.info(f"\nBlockers found, will re-review after next /develop pass...")

            # Commit any pending changes before re-review
